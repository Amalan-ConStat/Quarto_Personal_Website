...
[GitHub Code](https://github.com/Amalan-ConStat/TidyTuesday/tree/master/2019/Week10)

```{r}
#| label = "tweet",
#| echo = FALSE
blogdown::shortcode("tweet","1103226092352139264")
```
...
```{r}
#| label = "major category total workers"
jg_avg<-jobs_gender %>%
...
```{r}
#| label = "major category male workers"
p<-jobs_gender %>%
     select(-one_of(c("occupation","minor_category"))) %>%
     group_by(year,major_category) %>%
     summarise_all(funs(mean),na.rm=TRUE) %>%
...
```{r}
#| label = "major category female workers"
p<-jobs_gender %>%
     select(-one_of(c("occupation","minor_category"))) %>%
     group_by(year,major_category) %>%
     summarise_all(funs(mean),na.rm=TRUE) %>%
...
```{r}
#| label = "major category earnings male"
p<-jobs_gender %>%
     select(-one_of(c("occupation","minor_category"))) %>%
     group_by(year,major_category) %>%
     summarise_all(funs(mean),na.rm=TRUE) %>%
...
```{r}
#| label = "major category earnings female"
p<-jobs_gender %>%
     select(-one_of(c("occupation","minor_category"))) %>%
     group_by(year,major_category) %>%
     summarise_all(funs(mean),na.rm=TRUE) %>%
...
```{r}
#| label = "major category and wage percent for Female relative to Male"
p<-jobs_gender %>%
     select(-one_of(c("occupation","minor_category"))) %>%
     group_by(year,major_category) %>%
     summarise_all(funs(mean),na.rm=TRUE) %>%
...
```{r}
#| label = "minor category total workers"
p<-jobs_gender %>%
...
```{r}
#| label = "minor category male workers"
p<-jobs_gender %>%
     select(-one_of(c("occupation","major_category"))) %>%
     group_by(year,minor_category) %>%
     summarise_all(funs(mean),na.rm=TRUE) %>%
...
```{r}
#| label = "minor category female workers"
p<-jobs_gender %>%
     select(-one_of(c("occupation","major_category"))) %>%
     group_by(year,minor_category) %>%
     summarise_all(funs(mean),na.rm=TRUE) %>%
...
```{r}
#| label = "minor category total earnings male wage"
p<-jobs_gender %>%
     select(-one_of(c("occupation","major_category"))) %>%
     group_by(year,minor_category) %>%
     summarise_all(funs(mean),na.rm=TRUE) %>%
...
```{r}
#| label = "minor category total earnings female wage"
p<-jobs_gender %>%
     select(-one_of(c("occupation","major_category"))) %>%
     group_by(year,minor_category) %>%
     summarise_all(funs(mean),na.rm=TRUE) %>%
...
```{r}
#| label = "minor category and wage percent for Female relative to Male"
p<-jobs_gender %>%
     select(-one_of(c("occupation","major_category"))) %>%
     group_by(year,minor_category) %>%
     summarise_all(funs(mean),na.rm=TRUE) %>%
