...
```{r}
#| label = "Yearly shapes but decades"
...
```{r}
#| label = "Years of documentation and origin"
ufo_sightings %>%
...
```{r}
#| label = "Year with Encounter Length"
ggthemr("flat")
...
```{r}
#| label = "Hour of origin vs Encounter Length"
ufo_sightings %>%
  mutate(hour=hour(date_time)) %>%
  remove_missing() %>%
