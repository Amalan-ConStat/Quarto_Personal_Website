...
```{r}
#| echo = FALSE
blogdown::shortcode("tweet","1115652786862153729")
```
...
```{r}
#| label = "decade of birth vs first title won"
player_dob %>% 
   remove_missing() %>%
...
```{r}
#| label = "grand slams , Decade of Birth and age"
player_dob %>% 
  remove_missing() %>%
  mutate(grand_slam=recode_factor(grand_slam,
                                  'Wimbledon'="Wimbledon",
...
```{r}
#| label = "gender vs grand slam with name"
grand_slams %>%
...
```{r}
#| label = "gender vs grand slam with year"
p<-grand_slams %>%
...
```{r}
#| label = "Top 10 Tennis player and outcomes"
top10<-grand_slams %>%
       group_by(name) %>%
       count(sort = TRUE) %>%
       head(10) %>%
...
```{r}
#| label = "Top 10 Tennis Players and outome"
grand_slam_timeline %>%
      rename(name="player") %>%
      inner_join(top10,"name") %>%
...
```{r}
#| label = "Top 10 Tennis Players Winning"
grand_slam_timeline %>%
      rename(name="player") %>%
      inner_join(top10,"name") %>%
      subset(outcome=="Won") %>%
...
```{r}
#| label = "Top 10 Tennis Players Finalist"
grand_slam_timeline %>%
      rename(name="player") %>%
      inner_join(top10,"name") %>%
...
```{r}
#| label = "Top 10 Tennis Players Semi-Finalist"
grand_slam_timeline %>%
      rename(name="player") %>%
      inner_join(top10,"name") %>%
...
```{r}
#| label = "Top 10 Tennis Players Retired"
grand_slam_timeline %>%
      rename(name="player") %>%
      inner_join(top10,"name") %>%
