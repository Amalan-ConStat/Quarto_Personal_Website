...
```{r}
#| label = "Preparing to TidyTuesday"
#loading packages
#load data
library(readr)
#manipulate data
...
will get you with popularity and claps. The code will focus on Top 10 Authors with 
...
#| label = "my tweet",
#| echo = FALSE
blogdown::shortcode('tweet','1071369742672429056')
```

[GitHub Code](https://github.com/Amalan-ConStat/TidyTuesday/tree/master/Week_36)
...
```{r}
#| label = "Claps for Authors,titles,subtitles and Image"
...
```{r}
#| label = "Top 10 Authors in a table"
...
Extracting the top 10 authors with posts which have images it is clear
...
```{r}
#| label = "Top 10 Authors when there is Image"
...
```{r}    
#| label = "Top 10 Authors when there is No Image"
...
```{r}
#| label = "Top 10 Authors and reading time for their posts"
...
under a publication you will receive alot of claps because of the foundation these specific 
publications holds in Medium.

```{r}
#| label = "Top 5 publications in a table"
...
```{r}
#| label = "word cloud for Top 10 Authors",
#| fig.width = 5,
#| fig.height = 5
...
```{r}
#| label = "word cloud for Top 5 publications",
#| fig.width = 5.75,
#| fig.height = 5.75
#convert into data frame
...
* grid and gridExtra packages provide a safe way of combining multiple plots into
one plot.
...
* Word clouds for subtitles also will be interesting to see, specially focusing on authors 
and publications. 

_Please see that_      
This is my sixth post on the internet so my mistakes in grammar and spellings should be
very less than previous posts. I intend to post more statistics related materials 
